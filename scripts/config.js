...
// entry: 构建入口的js地址
// dest: 构建后的js文件地址
// format: 表示构建的格式 - cjs遵循CommonJS规范 - es 遵循ES Module规范 - umd遵循UMD规范

// Runtime + Compiler
// 在运行时对template进行编译，编译为render函数，所以需要带有编译器，编译过程对性能会有一定的损耗。
// Runtime Only
// 需要借助webpack的vue-loader工具把.vue文件编译成JavaScript，因为是在编译阶段做的，所以它只包含运行时的Vue代码，因此代码体积也会更轻。
